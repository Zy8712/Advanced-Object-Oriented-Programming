...
public final class Complex {
	...
	public Complex add(Complex c) {
		double readd = (this.re + c.re);
		double imadd = (this.im + c.im);

		return new Complex(readd, imadd);
	}
	...
	public Complex multiply(Complex c) {
		double remult = ((this.re * c.re) - (this.im * c.im));
		double immult = ((c.re * this.im) + (this.re * c.im));
	...
	@Override
	public int hashCode() {
		double valueReal = 9999 * this.re;
		double valueImag = 99 * this.im;
	...
	public static Complex valueOf(String s) {
		...
		if (parts.get(1).contentEquals("+") == false && parts.get(1).contentEquals("-") == false) {			
			throw new IllegalArgumentException("Not a complex number");
		}
