...
public class ExpressionCollector {
	...
	protected ArrayList<Expression> e;
	 ...
	 */
	ExpressionCollector() {
		this.e = new ArrayList<Expression>();
	}
	...
	void addExpression(int left, String op, int right) {
		...
		if (op.equals("+")) {
			this.e.add(new Addition(left, right));
		} else if (op.equals("-")) {
			this.e.add(new Subtraction(left, right));
		} else if (op.equals("*")) {
			this.e.add(new Multiplication(left, right));
		} else if (op.equals(">")) {
			this.e.add(new GreaterThan(left, right));
		} else if (op.equals("<")) {
			this.e.add(new LessThan(left, right));
		} else if (op.equals("==")) {
			this.e.add(new Equal(left, right));
		}
	...
	boolean isTypeCorrect() {
		...
		Expression expression = this.e.get(0);
		for (Expression exp : this.e) {
			if (exp.value.getClass() != expression.value.getClass()) {
				return false;
			}
		}
		return true;
	...
	Expression[] getExpressions() {
		...
		Expression[] result = new Expression[this.e.size()];
		for (int i = 0; i < result.length; i++) {
			result[i] = this.e.get(i);
		}
		return result;
	}
